...
const loadData = async () => {
    try {
        ...
        try {
            var backup = (typeof _tryRecoverFromBackup === 'function') ? _tryRecoverFromBackup() : null;
            if (backup && Array.isArray(backup.messages) && backup.messages.length > 0) {
                console.warn('[loadData] 主存储读取失败，从本地备份恢复');
                messages = backup.messages.map(function(m) { return Object.assign({}, m, { timestamp: new Date(m.timestamp) }); });
                if (backup.settings) settings = backup.settings;
                else settings = getDefaultSettings();
                setTimeout(function() { saveData(); }, 1000);
                showNotification('数据加载异常，已从备份恢复', 'warning', 5000);
            } else {
                settings = getDefaultSettings();
                messages = [];
                showNotification('数据加载失败且无可用备份', 'error', 5000);
            }
        } catch (e2) {
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            // ── 对方自发转账（约 0.8% 概率）──
            if (Math.random() < 0.008 && typeof window.partnerTransfer === 'function') {
                showTypingIndicator();
                var partnerName2 = settings.partnerName || '对方';
                var tiLabel2 = document.getElementById('typing-indicator-label');
                if (tiLabel2) tiLabel2.textContent = partnerName2 + ' 正在转账';
                var amounts = [5, 10, 20, 50, 52, 66, 88, 99, 100, 188, 200, 520];
                var amt = amounts[Math.floor(Math.random() * amounts.length)];
                var notes = ['请你喝奶茶', '给你买零食', '小小心意', '请你吃饭', '节日快乐', '今天开心', '送给你', '加油哦', '奖励', '爱你哟'];
                var nt = notes[Math.floor(Math.random() * notes.length)];
                var range = (settings.replyDelayMax || 3000) - (settings.replyDelayMin || 1000);
                var d = (settings.replyDelayMin || 1000) + Math.random() * range;
                setTimeout(function() {
                    (function(){try{if(window._typingIndicatorAutoHideTimer){clearTimeout(window._typingIndicatorAutoHideTimer);window._typingIndicatorAutoHideTimer=null;}}catch(e){}var _tiW=document.getElementById('typing-indicator-wrapper');if(_tiW){var _tiInner=_tiW.querySelector('.typing-indicator');if(_tiInner){_tiInner.classList.add('hiding');setTimeout(function(){_tiW.style.display='none';if(_tiInner)_tiInner.classList.remove('hiding');},240);}else{_tiW.style.display='none';}}})();
                    partnerTransfer(amt + Math.floor(Math.random() * 21), nt);
                }, d);
                return;
            }
