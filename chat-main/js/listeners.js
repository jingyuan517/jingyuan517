    ...
    return;
        ...
        function initModalListeners() {
            ...
            DOMElements.pokeModal.save.addEventListener('click', () => {
                const raw = DOMElements.pokeModal.input.value.trim() || settings.myPokeText || '';
                  let pokeText = raw || '拍了拍';
              ...
              document.querySelectorAll('.poke-mode-btn').forEach(btn => {
                  btn.addEventListener('click', function() {
                      document.querySelectorAll('.poke-mode-btn').forEach(b => {
                      ...
                      const isFree = this.dataset.mode === 'free';
                      document.getElementById('poke-free-hint').style.display = isFree ? 'block' : 'none';
                      document.getElementById('poke-template-hint').style.display = isFree ? 'none' : 'block';
                      const input = document.getElementById('poke-input');
                      input.placeholder = isFree ? '例如：我轻轻拍了拍你的头' : '例如：拍了拍（只填动作部分）';
                  });
              });
...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    if (myPokeTextInput) myPokeTextInput.value = settings.myPokeText || '';
      ...
      if (pokePreview) {
          const previewText = (settings.myPokeText || '拍了拍').replace(/\{我\}/g, settings.myName || '我').replace(/\{梦角\}/g, settings.partnerName || '对方');
          pokePreview.textContent = previewText;
      }
    if (myPokeTextInput) {
        myPokeTextInput.oninput = () => {
            const raw = myPokeTextInput.value.trim() || '拍了拍';
            const previewText = raw.replace(/\{我\}/g, settings.myName || '我').replace(/\{梦角\}/g, settings.partnerName || '对方');
            if (pokePreview) pokePreview.textContent = previewText;
          };
      }
...
playlist.style.top = (rect.top + (player.classList.contains('collapsed') ? 65 : 155)) + 'px';
    ...
    if (settings.musicPlayerEnabled) {
        ...
        function initCoreListeners() {
            ...
            DOMElements.plusBtn && (DOMElements.plusBtn.onclick = null);
            DOMElements.plusBtn && (DOMElements.plusBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                DOMElements.plusPopup.classList.toggle('active');
            }));
            DOMElements.plusImageBtn && (DOMElements.plusImageBtn.addEventListener('click', () => {
                DOMElements.plusPopup.classList.remove('active');
                if (typeof window.openImageUploadModal === 'function') window.openImageUploadModal();
            }));
            DOMElements.plusDecisionBtn && (DOMElements.plusDecisionBtn.addEventListener('click', () => {
                DOMElements.plusPopup.classList.remove('active');
                if (typeof openDecisionModal === 'function') openDecisionModal();
            }));
            document.addEventListener('click', (e) => {
                const wrapper = document.querySelector('.plus-btn-wrapper');
                if (wrapper && !wrapper.contains(e.target)) {
                    DOMElements.plusPopup && DOMElements.plusPopup.classList.remove('active');
                }
            });
