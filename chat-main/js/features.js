(function() {
    ...
    window._openPokeSymSettings = function() {
        ...
        function preview() {
            ...
            var mt   = ms?(ms+' 拍了拍你 '+ms):('拍了拍你');
            var pt   = ps?(ps+' 拍了拍你 '+ps):('拍了拍你');
...
function renderComboMenu() {
    ...
    tabBar.innerHTML = `
        ...
        <button class="combo-tab" data-tab="transfer" style="flex:1; padding:8px; border:none; background:var(--secondary-bg); color:var(--text-primary); border-radius:8px; cursor:pointer;">
            💸 转账
        </button>
    `;
    ...
    tabBar.querySelectorAll('.combo-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (btn.dataset.tab === 'emoji') {
                ...
                showPokeTab();
            } else if (btn.dataset.tab === 'transfer') {
                showTransferTab();
            }
        });
    });
...
function showTransferTab() {
    ...
    wrapper.innerHTML = `
        <div style="font-size:14px;font-weight:700;color:var(--text-primary);">趣味转账</div>
        <div style="font-size:12px;color:var(--text-secondary);">请输入金额（最大 99999999），可包含小数点，发送后会在聊天中显示转账信息作为趣味互动。</div>
        <div style="display:flex;gap:8px;align-items:center;">
            <input id="transfer-amount-input" type="text" inputmode="decimal" pattern="[0-9]*" placeholder="输入金额，例如：100 或 9.99" class="modal-input" style="flex:1;">
            <button id="transfer-send-btn" class="modal-btn modal-btn-primary" style="padding:8px 12px;">发送</button>
        </div>
        <div id="transfer-hint" style="font-size:12px;color:var(--text-secondary);"></div>
    `;

    area.appendChild(wrapper);

    const input = wrapper.querySelector('#transfer-amount-input');
    const sendBtn = wrapper.querySelector('#transfer-send-btn');
    const hint = wrapper.querySelector('#transfer-hint');
    ...
    function sanitizeAmount(v) {
        ...
        if (rounded > 99999999) return null;
        return rounded;
    }

    input.addEventListener('input', () => {
        const a = sanitizeAmount(input.value);
        if (a === null) {
            hint.textContent = '请输入有效数字且不超过 99999999';
        } else {
            hint.textContent = '将发送：¥' + a.toFixed(2);
        }
    });
    ...
    sendBtn.addEventListener('click', () => {
        const a = sanitizeAmount(input.value);
        if (a === null) {
            showNotification && showNotification('请输入有效金额，最大 99999999', 'warning');
            return;
        }
        const formatted = '💸 趣味转账 ¥' + a.toFixed(2) + ' (仅为互动演示)';
        ...
        setTimeout(() => {
            addMessage && addMessage({
                id: Date.now() + 1,
                sender: null,
                text: `对方收到了一笔 ¥${a.toFixed(2)} 的趣味转账，感谢互动 ✨`,
                timestamp: new Date(),
                type: 'system'
            });
        ...
        window.ensureTransferStyles && window.ensureTransferStyles();
    });
}

}

window.ensureTransferStyles = function() {
    if (document.getElementById('transfer-card-styles')) return;
    var s = document.createElement('style');
    s.id = 'transfer-card-styles';
    s.textContent = '.transfer-card-msg{display:flex;align-items:center;gap:10px;padding:2px 4px}.transfer-card-icon{width:40px;height:40px;border-radius:50%;background:#ff9500;display:flex;align-items:center;justify-content:center;flex-shrink:0}.transfer-card-icon i{color:#fff;font-size:20px}.transfer-card-body{display:flex;flex-direction:column}.transfer-card-label{font-size:11px;opacity:0.7;margin-bottom:1px}.transfer-card-amount{font-size:17px;font-weight:700}.transfer-card-note{font-size:12px;opacity:0.6;margin-top:2px}';
    document.head.appendChild(s);
};

window.openTransferModal = function() {
    var modal = document.getElementById('transfer-modal');
    var confirmModal = document.getElementById('transfer-confirm-modal');
    if (!modal) { if (typeof showNotification === 'function') showNotification('转账弹窗未找到', 'error'); return; }
    ...
    var pName = document.getElementById('transfer-partner-name');
    var pAvatar = document.getElementById('transfer-partner-avatar-img');
    if (pName) pName.textContent = partnerName;
    if (pAvatar && partnerAvatar) pAvatar.src = partnerAvatar.src;
    var img2 = document.getElementById('transfer-confirm-avatar-img');
    if (img2 && partnerAvatar) img2.src = partnerAvatar.src;

    var amountInput = document.getElementById('transfer-amount');
    var noteInput = document.getElementById('transfer-note');
    var submitBtn = document.getElementById('transfer-submit-btn');
    var hint = document.getElementById('transfer-amount-hint');
    ...
    function sanitize(v) {
        ...
        if (rounded > 999999999999) return null;
        return rounded;
    }

    function updateUI() {
        var a = sanitize(amountInput.value);
        if (a === null && amountInput.value.length > 0) {
            hint.textContent = '请输入有效金额';
            submitBtn.disabled = true;
            submitBtn.style.opacity = '0.4';
        } else if (a === null) {
            hint.textContent = '';
            ...
            hint.textContent = '金额不能为0';
            submitBtn.disabled = true;
            submitBtn.style.opacity = '0.4';
        } else {
            hint.textContent = '¥' + a.toFixed(2);
            submitBtn.disabled = false;
            submitBtn.style.opacity = '1';
        }
    }

    amountInput.addEventListener('input', updateUI, { signal: sig });
    ...
    submitBtn.addEventListener('click', function() {
        ...
        document.getElementById('transfer-confirm-name').textContent = partnerName;
        document.getElementById('transfer-confirm-label').textContent = '转账给';
        document.getElementById('transfer-confirm-amount').textContent = a.toFixed(2);
        document.getElementById('transfer-confirm-note').textContent = note || '趣味转账';

        setTimeout(function() {
            confirmModal.style.display = 'flex';
            var cc = confirmModal.querySelector('.modal-content');
            if (cc) { cc.style.opacity = '1'; cc.style.transform = 'translateY(0) scale(1)'; }

            var _confirmAbort = new AbortController();
            var confirmSig = _confirmAbort.signal;

            document.getElementById('transfer-confirm-cancel').addEventListener('click', function() {
                _confirmAbort.abort();
                confirmModal.style.display = 'none';
                if (cc) { cc.style.opacity = '0'; cc.style.transform = 'translateY(20px) scale(0.95)'; }
            }, { signal: confirmSig, once: true });

            document.getElementById('transfer-confirm-ok').addEventListener('click', function() {
                _confirmAbort.abort();
                confirmModal.style.display = 'none';
                if (cc) { cc.style.opacity = '0'; cc.style.transform = 'translateY(20px) scale(0.95)'; }
                executeTransfer(a, note);
            }, { signal: confirmSig, once: true });
        }, 200);
    }, { signal: sig });
};

window._buildTransferHtml = function(amount, note, label) {
    var noteHtml = note ? '<div class="transfer-card-note">' + note + '</div>' : '';
    return '<div class="transfer-card-msg"><div class="transfer-card-icon"><i class="fas fa-coins"></i></div><div class="transfer-card-body"><div class="transfer-card-label">' + label + '</div><div class="transfer-card-amount">¥' + amount.toFixed(2) + '</div>' + noteHtml + '</div></div>';
};

window.partnerTransfer = function(amount, note) {
    var partnerName = (typeof settings !== 'undefined' && settings.partnerName) || '对方';
    ensureTransferStyles && ensureTransferStyles();
    var formatted = _buildTransferHtml(amount, note, partnerName + '向你转账');
    addMessage && addMessage({
        id: Date.now() + '_transfer_' + Math.random().toString(36).slice(2,6),
        sender: 'partner',
        text: formatted,
        timestamp: new Date(),
        status: 'sent',
    ...
    setTimeout(function() {
        addMessage && addMessage({
            id: Date.now() + '_transfer_confirm_' + Math.random().toString(36).slice(2,6),
            sender: null,
            text: '<span style="display:inline-flex;align-items:center;gap:6px"><i class="fas fa-check-circle" style="color:#34c759;font-size:14px"></i> 已收款 ¥' + amount.toFixed(2) + '</span>',
            timestamp: new Date(),
            type: 'system'
        });
        if (typeof playSound === 'function') playSound('send');
...
function executeTransfer(amount, note) {
    var partnerName = (typeof settings !== 'undefined' && settings.partnerName) || '对方';
    ensureTransferStyles && ensureTransferStyles();
    var formatted = _buildTransferHtml(amount, note, '转账');
    addMessage && addMessage({
    ...
    setTimeout(function() {
        addMessage && addMessage({
            id: Date.now() + '_confirm_' + Math.random().toString(36).slice(2,6),
            sender: null,
            text: '<span style="display:inline-flex;align-items:center;gap:6px"><i class="fas fa-check-circle" style="color:#34c759;font-size:14px"></i> ' + partnerName + '已收款 ¥' + amount.toFixed(2) + '</span>',
            timestamp: new Date(),
            type: 'system'
        });
        if (typeof playSound === 'function') playSound('partner_message');
...
function showPokeTab() {
    ...
    const pokeList = Array.isArray(customPokes) && customPokes.length > 0 ? customPokes : (typeof CONSTANTS !== 'undefined' ? CONSTANTS.POKE_ACTIONS || [] : []);
    const displayPokes = pokeList.slice(0, 10);
    
    displayPokes.forEach(pokeText => {
        const displayText = String(pokeText || '');
        ...
        btn.onclick = () => {
            const msgText = displayText;
            addMessage({
                id: Date.now(), 
                text: _formatPokeText(msgText), 
                timestamp: new Date(), 
                type: 'system'
            });
            document.getElementById('user-sticker-picker').classList.remove('active');
    ...
    bottomRow.style.cssText = 'display:flex;gap:6px;margin-top:4px;';
    ...
    bottomRow.appendChild(customBtn);
    
    if (Array.isArray(customPokes) && customPokes.length > 0) {
        const manageBtn = document.createElement('button');
        manageBtn.innerHTML = '<i class="fas fa-list"></i> 管理库';
        manageBtn.style.cssText = `
            ...
            font-size: 13px;
        `;
        manageBtn.addEventListener('mouseover', () => { manageBtn.style.background = 'rgba(var(--accent-color-rgb),0.08)'; manageBtn.style.color = 'var(--accent-color)'; });
        manageBtn.addEventListener('mouseout', () => { manageBtn.style.background = 'var(--secondary-bg)'; manageBtn.style.color = 'var(--text-secondary)'; });
        manageBtn.onclick = () => {
            document.getElementById('user-sticker-picker').classList.remove('active');
            showPokeLibraryManager();
        };
...
function showPokeLibraryManager() {
    ...
    overlay.innerHTML = `
        <div style="background:var(--primary-bg);border-radius:20px;padding:20px;width:min(360px,92vw);max-height:80vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.28);border:1px solid var(--border-color);">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
                <span style="font-size:15px;font-weight:700;color:var(--text-primary);">拍一拍库</span>
                <button id="pml-close" style="background:none;border:none;font-size:18px;color:var(--text-secondary);cursor:pointer;padding:2px 6px;border-radius:6px;">✕</button>
            </div>
            <div style="font-size:12px;color:var(--text-secondary);margin-bottom:12px;">共 ${customPokes.length} 条拍一拍，点击删除</div>
            <div id="pml-list" style="flex:1;overflow:auto;display:flex;flex-direction:column;gap:6px;"></div>
        </div>
    `;
    document.body.appendChild(overlay);
    
    const list = overlay.querySelector('#pml-list');
    function renderPokeList() {
        list.innerHTML = '';
        if (!Array.isArray(customPokes) || customPokes.length === 0) {
            list.innerHTML = '<div style="text-align:center;color:var(--text-secondary);font-size:13px;padding:20px;">拍一拍库为空</div>';
            return;
        }
        customPokes.forEach((text, idx) => {
            const item = document.createElement('div');
            ...
            item.innerHTML = `
                <span style="flex:1;font-size:13px;color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${String(text||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;')}</span>
                  <button class="pml-del-btn" data-idx="${idx}" style="background:none;border:none;color:#ff5050;cursor:pointer;font-size:14px;padding:4px 6px;border-radius:6px;flex-shrink:0;">✕</button>
            `;
            const delBtn = item.querySelector('.pml-del-btn');
            delBtn.addEventListener('click', () => {
                customPokes.splice(idx, 1);
    ...
    overlay.querySelector('#pml-close').addEventListener('click', () => overlay.remove());
    overlay.addEventListener('click', (e) => { if (e.target === overlay) overlay.remove(); });
        ...
        window.openDecisionModal = function() {
            ...
            const questionInput = document.getElementById('decision-question-input');
            const optionsList = document.getElementById('decision-options-list');
            const addBtn = document.getElementById('decision-add-option-btn');
            const cancelBtn = document.getElementById('cancel-decision');
            const sendBtn = document.getElementById('send-decision');
            ...
            function renderOptions() {
                ...
                if (options.length === 0) {
                    ...
                    empty.style.cssText = 'text-align:center;color:var(--text-secondary);font-size:13px;padding:20px;';
                    empty.textContent = '点击"添加选项"来添加选项';
                    optionsList.appendChild(empty);
                } else {
                    options.forEach((opt, idx) => {
                        const row = document.createElement('div');
                        row.style.cssText = 'display:flex;align-items:center;gap:6px;padding:6px 10px;background:var(--secondary-bg);border-radius:8px;border:1px solid var(--border-color);';
                        row.innerHTML = '<span style="flex:1;font-size:13px;color:var(--text-primary);">' + String(opt).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') + '</span> <button class="decision-del-btn" style="background:none;border:none;color:#ff5050;cursor:pointer;font-size:14px;padding:2px 6px;border-radius:6px;">✕</button>';
                        row.querySelector('.decision-del-btn').addEventListener('click', () => {
            ...
            function updateSendBtn() {
                ...
                sendBtn.disabled = !(q && options.length >= 2);
            }
            ...
            questionInput.addEventListener('input', () => updateSendBtn(), { signal });

            addBtn.addEventListener('click', function addHandler() {
                const opt = prompt('请输入选项内容：');
                if (opt && opt.trim()) {
            ...
            const onSend = () => {
                ...
                const optionText = options.map((o, i) => String.fromCharCode(65 + i) + '. ' + o).join('\n');
                const msg = '帮我决定：' + q + '\n' + optionText;
                ...
                setTimeout(() => {
                    ...
                    const replyText = '关于"' + q + '"，我的决定是"' + chosen + '"';
                    addMessage({
                        id: Date.now(),
                        sender: 'partner',
                        text: replyText,
            ...
            cancelBtn.onclick = () => { ac.abort(); hideModal(modal); };
